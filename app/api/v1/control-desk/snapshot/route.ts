...
import { commerceInstallJobs, commerceLicenseInstallations, customers } from "../../../../../db/schema";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [customerRows, licenseRows, jobRows] = await Promise.all([
      customerScoped
        ? db.select({ id: customers.id, name: customers.name, company: customers.company, email: customers.email, status: customers.status }).from(customers).where(eq(customers.id, scopedCustomerId)).limit(1)
        : db.select({ id: customers.id, name: customers.name, company: customers.company, email: customers.email, status: customers.status }).from(customers).where(eq(customers.status, "active")).orderBy(desc(customers.updatedAt)).limit(500),
      customerScoped
    ...
    return controlDeskJson({
      ...
      installJobs: jobRows.map((job) => ({ id: job.id, jobId: job.jobId, domain: job.targetDomain, storeKey: job.storeKey, status: job.status, step: job.currentStep, lastSeenAt: job.updatedAt })),
