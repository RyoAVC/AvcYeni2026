...
export async function PATCH(
  ...
  if (!Number.isSafeInteger(id) || id < 1) return json({ ok: false, error: "Geçersiz modül numarası." }, 400);
  ...
  try {
    ...
    if (!existing) return json({ ok: false, error: "Modül bulunamadı." }, 404);
    ...
    const [slugTaken] = await db
      ...
      .from(modules)
      .where(and(eq(modules.slug, parsed.value.slug), ne(modules.id, id)))
      .limit(1);
    if (slugTaken) return json({ ok: false, error: "Bu kısa kod başka bir modülde kayıtlı." }, 409);
    ...
    const updated = await db.update(modules)
      ...
      .where(and(eq(modules.id, id), eq(modules.updatedAt, expectedUpdatedAt)))
      .returning({ id: modules.id, updatedAt: modules.updatedAt });
    ...
    console.error("Module update failed", cause);
    return json({ ok: false, error: "Modül şu anda güncellenemedi." }, 503);
  }
}
