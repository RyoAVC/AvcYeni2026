...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (existing) return json({ ok: false, error: "Bu kısa kod ile kayıtlı bir marka var." }, 409);
    ...
    await logAdminAction(db, {
      ...
      details: `Marka eklendi: ${name}`,
    });

    return json({ ok: true, id: inserted[0]?.id }, 201);
  } catch (cause) {
    console.error("Brand create failed", cause);
    return json({ ok: false, error: "Marka kaydedilemedi." }, 503);
  }
}
