...
export async function POST(request: Request) {
  ...
  try {
    ...
    const db = getDb();
    const [existing] = await db.select({ id: vitrineToasts.id }).from(vitrineToasts).where(eq(vitrineToasts.slug, parsed.value.slug)).limit(1);
    if (existing) return json({ ok: false, error: "Bu kısa kod ile kayıtlı bir bildirim var." }, 409);

    const now = new Date().toISOString();
    const inserted = await db.insert(vitrineToasts).values({
      ...parsed.value,
      createdAt: now,
      updatedAt: now,
    }).returning({ id: vitrineToasts.id });

    return json({ ok: true, id: inserted[0]?.id }, 201);
  } catch (cause) {
    console.error("Vitrine toast create failed", cause);
    return json({ ok: false, error: "Bildirim şu anda kaydedilemedi." }, 503);
  }
}
