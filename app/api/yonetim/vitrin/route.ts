...
export async function POST(request: Request) {
  ...
  try {
    ...
    const db = getDb();
    const [existing] = await db.select({ id: vitrineSignals.id }).from(vitrineSignals).where(eq(vitrineSignals.slug, parsed.value.slug)).limit(1);
    if (existing) return json({ ok: false, error: "Bu kısa kod ile kayıtlı bir vitrin satırı var." }, 409);

    const now = new Date().toISOString();
    const inserted = await db.insert(vitrineSignals).values({
      ...parsed.value,
      createdAt: now,
      updatedAt: now,
    }).returning({ id: vitrineSignals.id });

    return json({ ok: true, id: inserted[0]?.id }, 201);
  } catch (cause) {
    console.error("Vitrine signal create failed", cause);
    return json({ ok: false, error: "Vitrin satırı şu anda kaydedilemedi." }, 503);
  }
}
