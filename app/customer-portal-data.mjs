...
export async function loadCustomerPortalSnapshot(customer) {
  ...
  const [orderRows, ticketRows, invoiceRows, profileRows, moduleRows, integrationRows, metricRows, notificationRows, experimentRows, documentRows, moduleCatalogRows] = await Promise.all([
    ...
    db.select({ id: customerModuleInstances.id, key: modules.slug, name: modules.name, status: customerModuleInstances.status, coverage: customerModuleInstances.coverage, enabledAt: customerModuleInstances.enabledAt, expiresAt: customerModuleInstances.expiresAt, note: customerModuleInstances.note })
    ...
    db.select({ id: customerIntegrationInstances.id, providerKey: integrations.providerKey, name: integrations.name, category: integrations.category, status: customerIntegrationInstances.status, setupProgress: customerIntegrationInstances.setupProgress, healthScore: customerIntegrationInstances.healthScore, lastSyncAt: customerIntegrationInstances.lastSyncAt, lastErrorSummary: customerIntegrationInstances.lastErrorSummary, publicMetadata: customerIntegrationInstances.publicMetadata })
