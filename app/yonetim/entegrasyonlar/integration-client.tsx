...
export function IntegrationClient({ initialIntegrations }: { initialIntegrations: IntegrationItem[] }) {
  ...
  const activeCount = initialIntegrations.filter((item) => item.status === "active").length;
  const assignedCount = initialIntegrations.reduce((sum, item) => sum + item.customerActiveCount, 0);
  return <div className="integration-admin-stack">
    <aside className="integration-catalog-note"><div><span>AVCI COMMERCE BAĞLANTI KATALOĞU</span><strong>Mağazalara sunabileceğiniz tüm servisleri tek merkezden yönetin.</strong><p>Yayın durumu genel kataloğu, müşteri ataması ise ilgili mağazanın gerçek kullanımını belirler.</p></div><div className="integration-catalog-summary"><span><strong>{initialIntegrations.length}</strong> kayıt</span><span><strong>{activeCount}</strong> yayında</span><span><strong>{assignedCount}</strong> aktif kullanım</span></div><Link className="admin-btn admin-btn-primary" href="/yonetim/sistemler">Müşteriye bağlantı ata</Link></aside>
